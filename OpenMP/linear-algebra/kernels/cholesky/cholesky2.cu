                          ...
                          DATA_TYPE * __restrict__ A) 
{
  ...
  int j = tid + i + 1;
  __shared__ DATA_TYPE vec[BLOCK_SIZE];
  if (j >= n)
    return;
  ...
  for (int bk = 0; bk < i; bk += BLOCK_SIZE) {
    int index = bk + tid;
    if (index < n) {
      vec[threadIdx.x] = A[i*n + index];
    }

    __syncthreads();
    for (int k = bk; k < BLOCK_SIZE && k < i && j << n; k++) {
      tmp -= vec[k - bk] * A[j*n + k];
    }
                                ...
                                DATA_TYPE *A) 
{
  ...
  __shared__ DATA_TYPE tmp[BLOCK_SIZE];

  if (j < n && threadIdx.y == 0) {
    if (j < n)
        tmp[threadIdx.x] = A[i *n + j];
    else 
        tmp[threadIdx.x] = 0;  
  }

  for (int bk = 0; bk < i; bk += BLOCK_SIZE) {
    int index = bk + threadIdx.x;
    if (index < n && threadIdx.y == 0)
      vec_shared[threadIdx.x] = A[i * n + index];

    __syncthreads();

    if (bk + threadIdx.y < i && j < n) {
        atomicAdd(&tmp[threadIdx.x], -(vec_shared[threadIdx.y] * A[j*n + bk + threadIdx.y]));
    } 
...
int main(int argc, char **argv)
{
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (i < n - 1) {
      ...
      device_cholesky_2<<<numBlocks, BLOCK_SIZE>>>(n, i, p_d, A_d);
